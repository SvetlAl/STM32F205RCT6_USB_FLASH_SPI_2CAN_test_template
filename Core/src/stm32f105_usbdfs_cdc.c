...
static eDeviceState device_state = DEVICE_STATE_DEFAULT; /* Device state */
...
static inline void clear_device_status(eDeviceState state);

static inline uint32_t check_device_status(eDeviceState state);
...
#define CIRC_BUFFER_TX_SIZE 0x800UL
...
/*
static void write_to_circBuffer(uint8_t *buf, uint16_t len){
...
uint32_t USB_CDC_setTxBuffer(uint8_t EPnum, uint8_t *txBuff, uint16_t len){
	...
	if((EndPoint[EPnum].txCounter != 0) || (EndPoint[EPnum].statusTx == EP_ZLP) || (check_device_status(DEVICE_STATE_TX_PR) == EP_OK)){				
		return EP_FAILED;
	}
...
uint32_t USB_CDC_transmit_scheduler(){
	...
	if((is_tx_ep_fifo_ready(1, 1) == EP_OK) & (is_circBuffer_empty() !=0 )){
		...
		circBufferAddress newMsgaddr = read_circBuffer(256);
...
uint32_t USB_CDC_send_data(uint8_t *txBuff, uint16_t len){
	/******* DEVICE_STATE_READY ******/
	if((is_tx_ep_fifo_ready(1,1) == EP_OK) & (len!=0)){
