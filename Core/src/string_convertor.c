...
uint8_t halfbyte_to_hexascii(uint8_t _halfbyte){
	...
	_halfbyte &= 0x0F;
	if(_halfbyte >= 10){
		_ascii=(uint8_t)((uint8_t)'A' + _halfbyte - (uint8_t)10);
	} 
	else{ 
		_ascii=(uint8_t)((uint8_t)'0' + _halfbyte);
	}
	return _ascii;	
}
...
uint8_t hexascii_to_halfbyte(uint8_t _ascii){
	uint8_t _halfbyte;
	if((_ascii >= (uint8_t)'0') && (_ascii <= (uint8_t)'9')){
	_halfbyte=(uint8_t)(_ascii - (uint8_t)'0');}
	else if((_ascii >= 'a') && (_ascii <= (uint8_t)'f')){
//	_halfbyte=(_ascii - 'a');}
	_halfbyte=(uint8_t)(_ascii - (uint8_t)87);}
	else if((_ascii >= (uint8_t)'A') && (_ascii <= (uint8_t)'F')){
//	_halfbyte=(_ascii - 'A');}
	_halfbyte=(uint8_t)(_ascii - (uint8_t)55);}
	else{
		return(0xFF);
	}	
	return _halfbyte;
...
void str_to_ascii(uint8_t *data_in, uint8_t *data_out, uint16_t length){
	...
	for(uint32_t k = 0; k < length; k+=2){
		...
		data_out[i++] |= (uint8_t)((uint8_t)(cmd_byte << 4) & (uint8_t)~0x0F);
		} 	
}
