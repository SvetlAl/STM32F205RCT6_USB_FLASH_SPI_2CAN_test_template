...
static uint8_t elp_cmd_buffer[ELP_MAX_STR_CMD_LENGTH];
...
uint32_t pick_cmd(elp_cmd_BufferAddress addr){
	if((uint32_t)(addr.lastByte - addr.firstByte) < ((uint32_t)ELP_CMD_STRING_LENGTH -1)){
		global_el_command.cmd = ELP_CMD_ERROR;
		return ELP_ERROR;
	}
	else{
		for(uint32_t i = addr.firstByte; i < (addr.firstByte + ELP_CMD_STRING_LENGTH); i++){
...
uint32_t elp_parse_cmd(elp_cmd_BufferAddress addr){
	...
	for(uint32_t i = (addr.firstByte + ELP_CMD_STRING_LENGTH); i <= addr.lastByte; i++){
		/********* found something after '#' *********/
		if(elp_cmd_buffer[i-2] == '#'){
		...
		/*********** found a string start @!, make sure the size format is XX (two ascii) ***********/
			...
			if(!((uint32_t)(elp_cmd_buffer[i - (ELP_CMD_STRING_CMD_SIZ+1)]) | test_mask)){
				global_el_command.cmd = ELP_CMD_ERROR;
				return ELP_ERROR;
			}
...
uint32_t fill_elp_buffer(uint8_t *data, uint16_t length){
	...
	/****** idle routine ********/
	if(global_el_command.cmd == ELP_CMD_IDLE /* & predicted_index < ELP_MAX_STR_CMD_LENGTH */){
